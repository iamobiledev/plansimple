 ...
 *
 * A page's scale is stored as `pixelsPerUnit`: how many page coordinates
 * equal one base unit. Base units: feet (imperial) or meters (metric).
 * Geometry itself is stored in PDF points; calibration maps those to
 * real-world units at display / takeoff time.
 */
import type { MeasurementType, UnitSystem } from "../types.js";

/** pixels-per-unit from a calibration line of `pixelDistance` over `realLength` base units. */
