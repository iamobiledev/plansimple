...
// Bundler-agnostic worker resolution (webpack/Turbopack both understand
...
export function snapshotRegion(
  ...
  // JPEG keeps full-sheet snapshots well under serverless request-body limits.
  const dataUrl =
    format === "jpeg" ? canvas.toDataURL("image/jpeg", 0.85) : canvas.toDataURL("image/png");
  return { dataUrl, width, height, scale: outScale };
}
